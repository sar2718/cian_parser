...
def fetch_html(session, url, retries: int = FETCH_RETRIES, delay: int = FETCH_DELAY):
    for attempt in range(retries):
        try:
            ...
            if status == 429:
                print(f"\t⚠️ 429 Too Many Requests. Повторная попытка ({attempt+1}/{retries}). Ждём {wait_time:.1f} сек...")
                time.sleep(wait_time)
            elif status == 403:
                print(f"\t⚠️ 403 Forbidden. Меняем User-Agent и повторяем попытку ({attempt+1}/{retries})...")
                rotate_user_agent(session)
                time.sleep(wait_time)
            else:
                print(f"\t❌ Ошибка {e.response.status_code if e.response else '???'} при загрузке {url}")
                return None

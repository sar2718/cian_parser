...
def parse_detail_page(html: str) -> dict:
    ...
    data["price"] = int(''.join(price_container.get_text().split()[:-1])) if price_container else None
    ...
    for i in range(len(stations)):
        ...
        while "мин." not in times[i + time_i].get_text(strip=True):
            time_i += 1
        time_to_station = int(times[i + time_i].get_text().split()[0]) if (times[i + time_i].get_text().split()[0]).isdigit() else times[i + time_i].get_text(strip=True)
        data["metro"].append({
    ...
    for idx, group in enumerate(groups):
        ...
        if target == "apartment" and "Общая площадь" in data["attributes"][target]:
            if isfloat(data["attributes"][target]["Общая площадь"][:-2].strip().replace(',', '.')):
                data["attributes"][target]["Общая площадь"] = float(data["attributes"][target]["Общая площадь"][:-2].replace(',', '.'))
